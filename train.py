"""Step 8: run the MobileNetV2 experiment matrix and save results.

Trains augmented x unaugmented data on scratch / full_finetune / last_layer
strategies (6 runs total), evaluates each on the held-out test set, and
saves every model checkpoint plus a summary CSV to MODEL_DIR.
"""
import argparse
import os
import time
...
def run_experiments(fold=0, epochs=10, lr=1e-4):
    set_seed(SEED)
    ...
    for data_name in EXPERIMENT_DATASETS:
        ...
        for strategy in STRATEGIES:
            run_name = f"{data_name}_{strategy}"
            print("\n" + "=" * 70)
            ...
            model = build_mobilenet(strategy, num_classes, device)
            ...
            model_path = os.path.join(MODEL_DIR, f"{run_name}_mobilenetv2.pth")
    ...
    summary_path = os.path.join(MODEL_DIR, "mobilenet_experiment_summary.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train MobileNetV2 skin-lesion classifiers")
    parser.add_argument("--fold", type=int, default=0, help="Which CV fold to use as validation")
    parser.add_argument("--epochs", type=int, default=10)
    parser.add_argument("--lr", type=float, default=1e-4)
    args = parser.parse_args()

    run_experiments(fold=args.fold, epochs=args.epochs, lr=args.lr)
